...
public class MovieService {
    ...
    public Set<MovieDTO> getDanishMovieByAfterYear(String releaseDate, int totalNumberOfPages) {
        ...
        while (morePagesAvailable) {
            try {
                String url = "https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false" +
                        "&language=da-DK&page=" + currentPage + "&release_date.gte=" + releaseDate +
                ...
                if (response.statusCode() == 200) {
                    ...
                    for (MovieDTO movieDTO : moviesFromPage) {
                        ...
                        Set<GenreDTO> genreDTOSet = movieDTO.getGenre_ids().stream()
                                .map(id -> genreDTOs.get(id))
                                .collect(Collectors.toSet());
